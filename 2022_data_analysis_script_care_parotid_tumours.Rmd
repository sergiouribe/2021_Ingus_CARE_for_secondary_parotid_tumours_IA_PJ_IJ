...
df %>%
  ...
  distinct(., ID, .keep_all = TRUE) %>%   # filter unique IDs
  mutate(`Country of the correspondence author` = fct_lump_min(`Country of the correspondence author`, min = 7)) %>% 
  ggplot(aes(x = `YEAR published`)) + 
  geom_histogram(bins = 8, fill = "grey50") +
  facet_grid( `Country of the correspondence author` ~  .) + 
  labs(title = "Publications by corresponding author country", 
       x = "Year", 
       y = "Publications (log adjusted scale)") + 
  scale_y_log10()
